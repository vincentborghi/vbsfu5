...
console.log("PSM Helper Salesforce Full View: Core Logic Loaded.");
...
async function scrollToElementAndWait(selector, baseElement = document, timeout = 7000) {
    ...
    if (element) {
        ...
        // Wait for a second to ensure any lazy-loading triggered by scrolling has completed.
        await new Promise(resolve => setTimeout(resolve, 1200));
        console.log(`Successfully scrolled to and waited for: ${selector}`);
        return true;
    }
    console.warn(`Could not find element to scroll to: ${selector}`);
    return false;
}
...
async function findRecordNumber(baseElement) {
    ...
    if (!searchContext) {
        ...
        if (activeTabButton) {
            ...
            if (panelId) {
                ...
                if (searchContext) { 
                    console.log(`findRecordNumber: found panelId ${panelId} as search context ${searchContext}`); 
                }
    ...
    if (!searchContext) {
        console.warn('findRecordNumber: Could not determine a specific search context for record number. Defaulting to document.');
        searchContext = document;
        searchContext = document;
    } 

    console.log(`findRecordNumber: using search context ${searchContext}`);
    ...
    if (!detailsItem) { const detailsItem = searchContext.querySelector(woItemSelector); }
    ...
    } else { console.log("findRecordNumber: item/record number not found"); }
    return null;
}
...
async function processNotesList(baseElement) {
    ...
    if (!dataTable) {
        ...
        return [];
    } else {console.log("Found notes data table.");}
...
async function processWorkOrdersList(baseElement) {
    ...
    const listContainer = await waitForElement(containerSelector, baseElement);
    if (!listContainer) {
        console.log("Work Orders: Related list container NOT found. Assuming 0 Work Orders.");
        return { count: 0, html: '' };
    }

    const viewAllLink = listContainer.querySelector('a.slds-card__footer');
    if (!viewAllLink || viewAllLink.offsetParent === null) {
        console.log("Work Orders: 'View All' link not found or not visible. Assuming 0 Work Orders.");
        return { count: 0, html: '' };
    }
    ...
    // FIX: Corrected selector typo (missing closing parenthesis)
    const wocotainer = await waitForElement('lst-related-list-view-manager:has(span[title="Work Orders"])', document, 5000);
    if (wocotainer) {
        console.log("Work Orders: wocotainer Found ");
        searchContext = wocotainer;
    } else {
        console.log("Work Orders: wocotainer NOT Found ")
    }
    ...
    for (const selector of selectorsToTry) {
        console.log(`Work Orders: Attempting to find table with selector: "${selector}"`);
        ...
        if (foundElement) {
            if (foundElement.tagName.toLowerCase() === 'lightning-datatable') {
                ...
                if (tableElement) {
                    console.log(`Work Orders: Success! Found <table> inside <lightning-datatable>.`);
                    break;
                } else {
                    console.warn(`Work Orders: Found <lightning-datatable> but no <table> in its light DOM.`);
                    continue;
                }
            } else {
                tableElement = foundElement;
                console.log(`Work Orders: Success! Found table element directly with selector "${selector}".`);
                break;
            }
        }
    }
    ...
    if (!tableElement) {
        console.error("Work Orders data table not found after trying all selectors.");
        ...
        if (closeButton) {
            ...
            const modalDialog = document.querySelector('div.slds-modal[role="dialog"]');
            if (modalDialog) {
                const modalCloseButton = modalDialog.querySelector('button[title="Close"]');
    ...
    console.log("Work Orders: Successfully found data table element. Proceeding to scrape HTML.");
    ...
    if (closeButton) {
        ...
        if (modalDialog) {
            const modalCloseButton = modalDialog.querySelector('button[title="Close"]');
            if(modalCloseButton) {
                console.log('[DEBUG] Closing via generic modal close button.');
                modalCloseButton.click();
            } else {
                console.warn('Work Orders: Could not find a button to close the view.');
            }
        }
    }
    await new Promise(resolve => setTimeout(resolve, 1000));
...
async function showDebugInfo() {
    console.log("==== Hello from showDebugInfo");
    //console.log(`Active tab burron is: ${document.querySelector('li.slds-is-active[role="presentation"] a[role="tab"]')}`);
    let thisSelector = 'li.slds-is-active[role="presentation"] a[role="tab"]'
    const activeTabButton = document.querySelector(thisSelector);
    console.log(`Active tab button searched using ${thisSelector} is:\n ${activeTabButton}`);
    console.dir(activeTabButton);
    const panelId = activeTabButton.getAttribute('aria-controls');
    console.log(`panelId is: ${panelId}`);
    console.dir(activeTabButton);   
    const activeTabPanel = document.getElementById(panelId);
    console.log(`Active tab panel is:\n ${activeTabPanel}`);
    console.log("==== Bye from showDebugInfo");
    console.dir(activeTabPanel); 
}
...
async function generateCaseViewHtml(generatedTime) {
    ...
    // --- NEW: Generate Work Order Summary HTML block ---
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === "getCaseNumberAndUrl") {
        ...
        const recordNumber = activeTabPanel ? findRecordNumber(activeTabPanel) : null;
