...
function waitForElement(selector, baseElement = document, timeout = 5000) {
    return new Promise((resolve) => {
        ...
        const interval = setInterval(() => {
            ...
            if (element) {
                console.warn(`waitForElement: found element "${selector}"`);
                clearInterval(interval);
                resolve(element);
            } else if (Date.now() - startTime > timeout) {
                console.warn(`waitForElement: Timeout for "${selector}"`);
                clearInterval(interval);
                resolve(null);
            }
        }, 300);
...
async function injectCustomHeaderInfo() {
    ...
    if (!activeTabButton) {
        console.error("injectCustomHeaderInfo: Could not find the active Salesforce tab button.");
        if (statusDiv) {
    ...
    if (!anchorElement) {
        console.log('injectCustomHeaderInfo: Anchor element "div.slds-grid.primaryFieldRow" not found.');
        return;
    }
    ...
    // For Cases, the label is "Account Name" but for workorders it is "Account"
    ...
    // Get CreatedBy
    const createdByContainer = await waitForElement('records-record-layout-item[field-label="Created By"]', activeTabPanel);
...
function injectUI() {
    ...
    generateButton.onclick = async () => {
        ...
        try {
            ...
            // VB End
            await waitForElement('a.slds-card__header-link[href*="/related/PSM_Notes__r/view"]', document, 3000).then(el => el.scrollIntoView({ block: 'center' }));
            await new Promise(r => setTimeout(r, 500));
            await waitForElement('div.forceListViewManager[aria-label*="Emails"]', document, 3000).then(el => el.scrollIntoView({ block: 'center' }));
            await new Promise(r => setTimeout(r, 500));
            ...
            chrome.runtime.sendMessage({ action: "initiateGenerateFullCaseView" }, response => {
             ...
             // console.log("Error preparing page. ");
    ...
    copyButton.onclick = async () => {
        ...
        // Note: findRecordNumber() is defined in content.js and available globally here.
        ...
        if (currentUrl.includes('/Case/')) {
            ...
            // Optional: A default value if none of the conditions are met
            objectType = 'Unknown'; 
}
